...
export default class StartNewDeployment extends FTLActivity {
  ...
  run(cb: FTLRunCallback) {
    setTimeout(() => {
      this.logger.info(`startNewDeployment ran ${this.mult}`);
      cb(null, {hooray: 'yall'}, {startNewDeployment: Math.round(Math.random() * this.mult)});
    }, 1000);
  }

  status(): any {
    return 'startNewDeployment';
  }
